...
export const ExpandedCard: FC<ExpandedCardProps> = ({
    poster,
    title,
    rating,
    releasedYear,
    ...
    return (
        <div className="bg-[#394B61] h-full flex flex-row rounded-[11px] overflow-hidden">
            <img 
                className={`w-40 sm:w-56 xl:w-[340px] h-full object-cover transition-opacity
                    ${showPoster ? ' opacity-100 fadeInOut' : ' opacity-0'}
                `}
                src={poster[imageIndex]}
                alt={title}
            />

            <div className="animated-card-right-content flex-1 p-3.5 sm:p-6 xl:pl-11 xl:pr-11 xl:pt-8 xl:pb-6 space-y-3 xl:space-y-6">
